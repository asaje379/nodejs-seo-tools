...
export class InternalLink {
  ...
  formatCrawlerOptions(uriTab: string[], currentIndex: string) {
    return uriTab.map((uri: string) => {return {uri: currentIndex + uri, index: `${currentIndex === '' ? '' : '[' + currentIndex + ']'}['${uri}']`}});
  }
  ...
  const crawler = new Crawler({
    ...
    callback: (error, res, done) => {
      if (error) {
        ...
        let internalLinksOnPage = res.$('a[href^="/"]').toArray().map((link) => {
        ...
        internalLinks.push({ url: currentPageUrl, links: internalLinksOnPage });
